//apps/dashboard/src/lib/ecommerceConnectors.ts
import type { LucideIcon } from 'lucide-react';
import { Globe, KeyRound, ShoppingBag, Store, Tag } from 'lucide-react';

export type EcommercePlatform = 'shopify' | 'adobe_commerce' | 'bigcommerce';
...
export const connectorsConfig: ConnectorsConfig = {
  ...
  bigcommerce: {
    ...
    help: {
      ...
      docsUrl: 'https://developer.bigcommerce.com/api-docs/getting-started/authentication',
      docsLabel: 'BigCommerce API auth docs'
    },
    fields: [
      {
        id: 'storeHash',
        ...
        info: 'The BigCommerce Store Hash from your API Accounts settings (required). Do NOT use the storefront URL.'
      },
      {
        id: 'storeUrl',
        ...
        info: 'Your BigCommerce storefront URL for reference (optional).'
      },
      {
        id: 'accessToken',
        ...
        type: 'password',
        icon: KeyRound,
        info: 'API account token used to authenticate requests to the BigCommerce API (required).'
      }
    ]
  }
};
...
const createStoreRecordSet = (platform: EcommercePlatform, connectorId: string, values: ConnectorSetupValues, projectId: string): ConnectedStore => {
  ...
  const baseUrl = platform === 'shopify'
    ? `https://${values.shopDomain || 'new-store.myshopify.com'}`
    : platform === 'bigcommerce'
      ? values.storeUrl?.trim() || `https://store-${String(values.storeHash || 'hash').trim()}.mybigcommerce.com`
      : values.storeUrl || 'https://new-store.example.com';
  ...
  return {
    ...
    tokenExpiresAt: new Date(
      Date.now() +
        1000 * 60 * 60 * 24 * (platform === 'shopify' ? 45 : platform === 'bigcommerce' ? 30 : 16),
    ).toISOString(),
    recordsSyncedLast24h: platform === 'shopify' ? 320 : platform === 'bigcommerce' ? 280 : 256,
    name: platform === 'shopify'
      ? values.shopDomain || 'new-store.myshopify.com'
      : platform === 'bigcommerce'
        ? (values.storeHash || 'BigCommerce Store')
        : values.storeCode
          ? `${values.storeCode} Store`
