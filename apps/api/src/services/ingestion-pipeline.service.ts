...
export class IngestionPipeline {
    ...
    public static async intake(envelope: IngestionEnvelope): Promise<IngestionEventRecord> {
        ...
        // Store raw ingestion event in database
        await prisma.ingestionEvent.create({
            data: {
                id: eventRecord.id,
                tenantId: envelope.tenantId,
                projectId: envelope.projectId,
                connectorInstanceId: envelope.integrationId,
                eventType: envelope.mode === 'WEBHOOK' ? 'WEBHOOK_RECEIVED' : 'API_CALL',
                source: envelope.connectorType || 'UNKNOWN',
    ...
    private static async checkDeduplication(envelope: IngestionEnvelope): boolean {
        // Logic: Check if sourceEventId + integrationId has been seen in last 24h
        if (!envelope.sourceEventId) return false;
