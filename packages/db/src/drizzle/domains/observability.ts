...
export const systemHealthMetrics = pgTable('system_health_metrics', {
    ...
    projectTsIdx: index('idx_health_project_ts').on(table.projectId, table.timestamp),
}));
...
export const connectorHealthSnapshots = pgTable('connector_health_snapshots', {
    ...
    projectTsIdx: index('idx_health_shot_project_ts').on(table.projectId, table.timestamp),
}));
